...
## Fase 44: Performance Mobile 66 → 75+ (Mar 2026)
- [x] Inline critical CSS no head para eliminar render-blocking (740ms) — 12.9KB inlined, CSS async via preload+onload
- [ ] Adicionar loading="lazy" decoding="async" em TODAS as imagens below-fold
- [x] Corrigir canonical URL conflitante (drudialmeida.com.br → drudiealmeida.com.br) — SEO score 92→100
- [x] Integrar extract-critical-css.py no pipeline de build (package.json)
- [ ] Reduzir DOM size: deferir seções below-fold com IntersectionObserver
- [ ] Otimizar carrossel de convênios (remover duplicação 3x dos logos)
- [ ] Verificar e adicionar width/height em todas as imagens sem dimensões explícitas
